...
const IMAGES = [
  './img/image01.webp', './img/image02.webp', './img/image03.webp', './img/image04.webp',
  './img/image05.webp', './img/image06.webp', './img/image07.webp', './img/image08.webp',
  './img/image09.webp', './img/image10.webp'
];
...
function drawBackground() {
  ...
  // Render faster while transitioning or when user interacts quickly
  const quick = (now < bgFastUntil) || (Math.abs(vX) > 28);
  ...
  // Interactive swirl based on scroll position and drag velocity
  const trackNorm = TRACK ? (SCROLL_X / TRACK) : 0;
  const swirl = trackNorm * Math.PI * 2;
  const drift = time * 0.22; // constant drift to ensure continuous motion
  const sway = Math.min(0.45, Math.abs(vX) / 400);
  ...
  const x2 = cx + Math.cos(-time * 0.9 + 1.2 - swirl * 1.1 + drift * 0.6) * a2 * (1 + sway * 0.4);
  const y2 = cy + Math.sin(-time * 0.7 + 0.7 - swirl * 0.8 + drift * 0.5) * a2 * 0.5 * (1 + sway * 0.4);
  const x3 = cx + Math.cos(time * 1.6 - swirl * 0.8 + 0.7 + drift * 1.1) * a3 * (1 + sway * 0.3);
  const y3 = cy + Math.sin(time * 1.2 - swirl * 0.6 + 1.7 + drift * 0.9) * a3 * 0.6 * (1 + sway * 0.3);
  const x4 = cx + Math.cos(-time * 1.3 + swirl * 1.4 + 2.2 + drift * 0.8) * a4 * (1 + sway * 0.2);
  const y4 = cy + Math.sin(-time * 1.5 + swirl * 1.1 + 0.9 + drift * 0.6) * a4 * 0.7 * (1 + sway * 0.2);
